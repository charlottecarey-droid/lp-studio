...
BEGIN
  ...
  IF existing_config IS NULL THEN
    -- No brand settings row for Dandy yet (shouldn't happen, but be safe).
    INSERT INTO lp_brand_settings (tenant_id, config)
    VALUES (1, jsonb_build_object('salesConsole', '{}'::jsonb));
    SELECT config INTO existing_config FROM lp_brand_settings WHERE tenant_id = 1 LIMIT 1;
  END IF;
