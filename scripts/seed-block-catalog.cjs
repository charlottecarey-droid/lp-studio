#!/usr/bin/env node
/**
 * Seed the block_catalog table with initial rows.
 *
 * - For industry "generic": insert curated reusable DSO blocks + universal blocks
 *   with B2B SaaS placeholder copy override (partial overrides; merged with
 *   in-code BLOCK_REGISTRY defaults at read time on the frontend).
 * - For industry "dental": no rows are seeded — frontend falls back to BLOCK_REGISTRY.
 *
 * Idempotent: uses ON CONFLICT DO NOTHING so reruns won't clobber admin edits.
 *
...
// ─── Generic industry seed ───────────────────────────────────────────────────
// Each row: { block_type, label, category, default_props (overrides), sort_order }
// default_props is a *partial* override. Empty {} means "use registry default
// as-is". The frontend merges these on top of the in-code default.
const GENERIC_SEED = [
  // ── Layout ─────────────────────────────────────────────────────────────────
  { block_type: "hero",            label: "Hero",             category: "Layout",       sort_order: 10,
    default_props: {
  ...
  { block_type: "full-bleed-hero", label: "Full-Bleed Hero",  category: "Layout",       sort_order: 11, default_props: {
  ...
  { block_type: "photo-strip",     label: "Photo Strip",      category: "Layout",       sort_order: 12, default_props: {} },
  ...
  { block_type: "product-grid",    label: "Product Grid",     category: "Content",      sort_order: 24, default_props: {
  ...
  { block_type: "product-showcase",label: "Product Showcase", category: "Content",      sort_order: 25, default_props: {} },
  { block_type: "zigzag-features", label: "Zigzag Features",  category: "Content",      sort_order: 26, default_props: {} },
  ...
  { block_type: "cta-button",      label: "CTA Button",       category: "CTA",          sort_order: 51, default_props: {} },

  // ── Lead Capture ───────────────────────────────────────────────────────────
  { block_type: "form",            label: "Form",             category: "Lead Capture", sort_order: 60, default_props: {} },

  // ── Engagement ─────────────────────────────────────────────────────────────
  { block_type: "popup",           label: "Popup",            category: "Engagement",   sort_order: 70, default_props: {} },
  { block_type: "sticky-bar",      label: "Sticky Bar",       category: "Engagement",   sort_order: 71, default_props: {} },
  { block_type: "sticky-header",   label: "Sticky Header",    category: "Engagement",   sort_order: 72, default_props: {} },
  { block_type: "nav-header",      label: "Nav Header",       category: "Layout",       sort_order: 14, default_props: {} },
  { block_type: "footer",          label: "Footer",           category: "Layout",       sort_order: 15, default_props: {} },

  // ── Interactive ────────────────────────────────────────────────────────────
  { block_type: "roi-calculator",  label: "ROI Calculator",   category: "Interactive",  sort_order: 80, default_props: {} },
  ...
  { block_type: "dso-final-cta",   label: "Final CTA",               category: "CTA",          sort_order: 52, default_props: {} },
  ...
  { block_type: "dso-comparison",  label: "Animated Comparison",     category: "Content",      sort_order: 43,
    default_props: { headline: "A modern stack vs. the old way." } },
  ...
  { block_type: "dso-pilot-steps", label: "Pilot Steps",             category: "Content",      sort_order: 46, default_props: {} },

  { block_type: "dso-scroll-story",label: "Scroll Story",            category: "Engagement",   sort_order: 73, default_props: {} },
  { block_type: "dso-scroll-story-hero", label: "Scroll Story Hero", category: "Engagement",   sort_order: 74, default_props: {} },
  { block_type: "dso-particle-mesh", label: "Particle Mesh",         category: "Engagement",   sort_order: 75, default_props: {
  ...
  { block_type: "dso-network-map", label: "Network Map",             category: "Engagement",   sort_order: 76,
    default_props: { eyebrow: "Global", headline: "Customers in 40+ countries", body: "Built to scale wherever your team is." } },
  { block_type: "dso-live-feed",   label: "Live Feed",               category: "Engagement",   sort_order: 77, default_props: {} },
  { block_type: "dso-case-flow",   label: "Case Flow",               category: "Engagement",   sort_order: 78, default_props: {} },
  ...
  try {
    for (const row of GENERIC_SEED) {
      const r = await pool.query(
        ...
        [row.block_type, row.label, row.category, JSON.stringify(row.default_props ?? {}), row.sort_order ?? 0]
      );
