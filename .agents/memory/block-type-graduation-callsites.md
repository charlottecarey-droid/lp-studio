...
**ICON_ONLY_ITEM_BLOCK_TYPES gates per-item photos (generate-page.ts).** A graduated SECTION block whose items render a real photo (with an icon fallback) must be kept OUT of `ICON_ONLY_ITEM_BLOCK_TYPES` — that set gates ALL THREE image passes (`collectImageSlots`, `fillEmptyImages`, `aiFillEmptyImages`) for `items[].image`, so a photo-capable block placed there renders empty cards ("white on white, no images") on AI-generated pages. Only genuinely icon-led item blocks (e.g. value-pillars icon-trio / outlined-cards / divided-columns / card-columns) belong in the set. **Why:** the gate exists to stop fabricated logo/photo fill on icon-led blocks; it silently starves photo blocks of imagery. **How to apply:** when graduating a photo-capable items[] section block, (1) leave it out of ICON_ONLY, (2) add `image ("" — server fills)` to its AI prompt-schema bullet so the model emits the empty key the fill loop keys on (`"image" in item`), and (3) non-benefits item blocks fill with purpose `product-detail` (biasPage=false) — acceptable; lp-feature library/AI fill still backstops.
