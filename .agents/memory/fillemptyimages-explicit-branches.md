...
**The `items[].image` gate is family-conditional, unlike the products rule above.** Photo-LED section families (feature-* + the two photo-led value-pillars) use the `!item.image` truthiness gate so an OMITTED image key still backfills. But GENERIC item blocks whose items carry no photo keep the stricter `"image" in item && !item.image` guard — otherwise the fill would attach an image slot to a block that has no photo frame. So "never gate on `in`" is specific to arrays where EVERY item is meant to have a photo (products, photo-led sections), not a universal law. The icon-LED value-pillars sections are excluded entirely (they're in `ICON_ONLY_ITEM_BLOCK_TYPES`).
