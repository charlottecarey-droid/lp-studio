...
**Exception — THIS run's scrapes are TRUSTED (the curated gate).** A scrape harvested from the reference URL(s) the user pointed us at *this generation* — `freshScrapedMedia`, OR a catalog scrape whose `refhost:` matches a current reference host — passes the strict gate on `score >= 0` like a curated image, even with no lexical title overlap. The user explicitly referenced that site, so its imagery should fill empty slots before AI generation. Stale/other-host scrapes from prior generations keep the strict `> 0` gate. Implemented via `buildTrustedScrapedIds(catalogImages, freshScrapedMedia, referenceUrls)` → a `Set` threaded into `fillEmptyImages(..., trustedScrapedIds)` → `findBestImage`. The set is keyed by `imageIdentity` (scrapes fold to `s:<host>:<title-stem>`, NOT the URL) — assert in tests via `.size` or end-to-end `fillEmptyImages` behavior, never `set.has("/objects/...")`. Both generators must build the pool with `buildReferenceFillPool` AND the trust set with `buildTrustedScrapedIds` at every STRICT `fillEmptyImages` callsite (relaxed passes ignore the gate, so no trust set needed there).
