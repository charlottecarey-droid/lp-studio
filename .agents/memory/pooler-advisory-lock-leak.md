...
**Recurred in the deploy migrate path (durable fix shipped):** a killed/cancelled publish left the **migration** advisory lock held on a `-pooler` backend with `state='active'`. The prod auto-steal deliberately won't terminate `active` holders (could be a real peer mid-migrate), so the migrate step hard-failed every retry until a manual `pg_terminate_backend`. Clear it via `pg_locks` filtered to the migration key (`classid`,`objid` = the bigint split), then re-publish. **Fix:** `runMigrationsLocked` now holds its lock client on the Neon **direct** endpoint via `toDirectConnectionString` (strips `-pooler` suffix from the FIRST host label, `*.neon.tech`-scoped only) so the session lock releases the instant the socket closes — no pooler keeps the backend alive. Migration DDL still runs over the pooled `db`/`pool` (drizzle wraps it in one txn-scoped transaction, pooler-safe).
