...
export const LP_PUBLIC: { method: string; pattern: RegExp }[] = [
  ...
  { method: "GET",  pattern: /^\/lp\/plan-config$/ },
  { method: "GET",  pattern: /^\/lp\/beta-offer$/ },    // GET /lp/beta-offer — public founding-beta spots remaining (marketing site); reports the same cap signup enforces     // GET /lp/plan-config — public, live (SuperAdmin-editable) plan/pricing config for app billing/upgrade surfaces
