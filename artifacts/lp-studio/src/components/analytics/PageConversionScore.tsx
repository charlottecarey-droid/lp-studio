...
async function fetchScoring(pageId: number): Promise<ScoringResult> {
  const r = await fetch(`${API_BASE}/lp/conversion-scoring/${pageId}`);
  if (!r.ok) throw new Error(`HTTP ${r.status}`);
  return r.json() as Promise<ScoringResult>;
}

export function PageConversionScore({ pageId }: { pageId: number }) {
  const { data: result, isLoading, isError } = useQuery({
    queryKey: ["page-conversion-score", pageId],
    queryFn: () => fetchScoring(pageId),
    ...
    return (
      <Card className="p-6">
        <div className="text-center py-6">
          ...
          <p className="text-sm text-muted-foreground">Could not load conversion score for this page.</p>
        </div>
      </Card>
    );
  }
...
export default PageConversionScore;
