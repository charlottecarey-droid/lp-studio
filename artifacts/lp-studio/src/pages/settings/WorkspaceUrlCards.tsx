...
import { Loader2, Globe, Copy, Check, AlertCircle, CheckCircle2, Trash2 } from "lucide-react";
import { useToast } from "@/hooks/use-toast";

/**
 * Workspace-URL cards: the canonical login URL (read-only), the slug editor,
 * and the active slug-redirect manager.
 *
 * Lifted verbatim from GeneralPage in settings consolidation Phase 3 so every
 * domain/URL concept lives on the Domains tab: landing-page domain (custom +
 * managed), workspace sign-in URL (these cards), and the email sending domain
 * (linked out to Settings → Email → Sending). All three cards are
 * self-fetching and prop-less.
 */
...
// Task #132 — show the canonical workspace URL so members always know where
// to bookmark / share for sign-in. Server computes this from the tenant slug
// + the wildcard base host config, so it stays correct even when the wildcard
// base changes.
export function WorkspaceLoginUrlCard() {
  const { user } = useAuth();
...
export function ActiveRedirectsCard() {
  const { user } = useAuth();
  const { toast } = useToast();
  const isAdmin = user?.isAdmin ?? false;
