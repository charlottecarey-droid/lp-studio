...
const SalesRoiCalculator = () => {
  ...
  const exportPDF = useCallback(() => {
    ...
    // pass it through to the PDF builder so the header/footer wordmark
    // matches their brand. Empty/missing logo falls back to a text wordmark.
    if (!isDandy) {
      if (brand.logoUrl) {
        const img = new Image();
        ...
        img.onload = () => {
          try {
            ...
            const scale = 2;
            const targetW = 103 * scale;
            const aspect = img.naturalHeight > 0 ? img.naturalWidth / img.naturalHeight : 103 / 37;
    ...
    function buildPDF(logoData: string) {
      ...
      if (logoData) {
        try { doc.addImage(logoData, "PNG", marginL, 20, logoW, logoH); } catch {
          doc.setFont("helvetica", "bold"); doc.setFontSize(18); doc.setTextColor(...white);
          doc.text(brand.brandName || "", marginL, 38);
        }
      ...
      if (logoData) {
        try { doc.addImage(logoData, "PNG", marginL, pageH - footerH + 10, 48, 17); } catch {
          if (brand.brandName) {
            doc.setFont("helvetica", "bold"); doc.setFontSize(12); doc.setTextColor(...white);
            doc.text(brand.brandName, marginL, pageH - footerH + 24);
          }
        }
      } else if (brand.brandName) {
        doc.setFont("helvetica", "bold"); doc.setFontSize(12); doc.setTextColor(...white);
      ...
      // Centre-footer URL: Dandy keeps its historical link; non-Dandy tenants
      // get the brand's CTA URL (defaultCtaUrl) when set, else nothing.
      const footerCenter = isDandy
        ? "www.meetdandy.com/dso"
        : (brand.defaultCtaUrl && brand.defaultCtaUrl !== "#" ? brand.defaultCtaUrl.replace(/^https?:\/\//, "") : "");
      if (footerCenter) {
        doc.text(footerCenter, pageW / 2, pageH - footerH + 22, { align: "center" });
      }
      doc.setTextColor(...lime);
      doc.setFontSize(8);
