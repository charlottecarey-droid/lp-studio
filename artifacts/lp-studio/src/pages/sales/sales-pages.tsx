...
export default function SalesPages() {
  ...
  // The modal itself lives in components/sales/EmailPreviewModal.tsx so the
  // other envelope buttons (Accounts → microsites) open the same thing.
  ...
  return (
    <SalesLayout>
                            ...
                            <td className="px-3 py-3 whitespace-nowrap">
                              <div className="flex items-center justify-end gap-0.5" onClick={(e) => e.stopPropagation()}>
                                ...
                                <Button
                                  ...
                                  onClick={() => setEmailPreviewModal(row)}
                                >
      ...
      {/* ── Copy email preview (choose plain vs personalized link) ──────── */}
      <EmailPreviewModal
        page={emailPreviewModal && {
          pageId: emailPreviewModal.pageId,
          pageTitle: emailPreviewModal.pageTitle,
          plainUrl: getLpPageUrl(emailPreviewModal.pageSlug, micrositeDomain, tenantHost),
          hotlinks: emailPreviewModal.hotlinks,
        }}
        onClose={() => setEmailPreviewModal(null)}
        onHotlinkCreated={(pageId, hl) => {
          const entry: HotlinkEntry = { ...hl, contactName: hl.contactName ?? "" };
          setEmailPreviewModal(prev => prev && prev.pageId === pageId ? { ...prev, hotlinks: [...prev.hotlinks, entry] } : prev);
          setRows(prev => prev.map(r => r.pageId === pageId ? { ...r, hotlinks: [...r.hotlinks, entry] } : r));
        }}
      />
