...
import { Check, RotateCcw, Save, Settings2, BookmarkPlus, ShieldCheck } from "lucide-react";
...
import {
  ...
  isDefaultGovernanceEntry,
  type WorkingGovernanceEntry,
...
export default function BlockDefaultsPage() {
  // Standalone /block-defaults route — same editor as the embedded
  // BlockDefaultsContent (used in Blocks settings), wrapped in the app shell.
  return (
    <AppLayout>
      <div
...
/**
 * Per-block Governance tab (task #6) rendered inside the Block Defaults editor
 * next to Content / Style. Edits the SAME tenant governance record as the
 * standalone Block Governance panel — preview, customize, and control in one
 * place. Uses the shared `BlockGovernanceControls` so the two surfaces never
...
function BlockGovernanceTab({
  ...
  save: (next: TenantBlockGovernanceEntry[]) => Promise<boolean>;
  saving: boolean;
  loading: boolean;
}) {
  const { toast } = useToast();
  const [working, setWorking] = useState<WorkingGovernanceEntry | null>(null);

  const saved = useMemo<WorkingGovernanceEntry>(() => {
    const e = entries.find((x) => x.blockType === blockType);
    return e
      ? { enabled: e.enabled, aiMode: e.aiMode, segments: new Set(e.segments) }
      : { enabled: null, aiMode: "open", segments: new Set<string>() };
  }, [entries, blockType]);

  // Drop in-flight edits when the selected block changes or governance reloads.
  useEffect(() => {
    setWorking(null);
  }, [blockType, entries]);

  const entry = working ?? saved;
  const dirty = working !== null;

  const handleSave = async () => {
    const payload: TenantBlockGovernanceEntry[] = entries.filter((e) => e.blockType !== blockType);
    if (!isDefaultGovernanceEntry(entry)) {
      payload.push({
        blockType,
        enabled: entry.enabled,
        aiMode: entry.aiMode,
        segments: Array.from(entry.segments),
      });
    }
    const ok = await save(payload);
    if (ok) {
      setWorking(null);
      toast({ title: "Governance saved", description: `${blockLabel} governance updated for your team.` });
    } else {
  ...
  if (loading) {
    return <p className="text-sm text-muted-foreground py-4">Loading governance…</p>;
  }

  return (
    <div className="space-y-5">
      <p className="text-xs text-muted-foreground leading-relaxed">
        Control whether your team can use this block, how AI may edit it, and
        which audience segments it belongs to. These settings apply everywhere
        this block is used — the same record as the Block Governance overview.
      </p>
      ...
      <BlockGovernanceControls
        ...
        onChange={setWorking}
        layout="stack"
        enableLabel={blockLabel}
      />

      {segments.length === 0 && (
        <p className="text-[11px] text-muted-foreground">
          Add audience segments in Brand settings to approve this block per segment.
        </p>
      )}

      <div className="pt-3 border-t border-border">
        <Button size="sm" className="w-full gap-1.5" onClick={handleSave} disabled={!dirty || saving}>
...
export function BlockDefaultsContent() {
  ...
  // Every remaining category in the registry, so EVERY block is configurable
  // here — not just the curated core/segment sets. Derived dynamically so new
  // block categories surface automatically.
  const otherCategories = useMemo<BlockCategory[]>(() => {
    ...
    <div className="flex h-full">
      ...
      <aside className="w-64 shrink-0 border-r border-border bg-background flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-4">
          ...
          {/* Every remaining block category, so nothing is unreachable */}
          {otherCategories.length > 0 && (
            <div className="border-t border-border pt-3">
              <div className="flex items-center gap-1.5 px-2 mb-2">
                <div className="w-1.5 h-1.5 rounded-full bg-muted-foreground/50" />
                <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Other Blocks</p>
              </div>
              <div className="space-y-4">
