import type { GalleryFilmstripBlockProps, GalleryImage } from "@/lib/block-types";
...
interface Props {
  ...
  onChange: (next: GalleryFilmstripBlockProps) => void;
}

export function GalleryFilmstripPanel({ props, onChange }: Props) {
  const update = (patch: Partial<GalleryFilmstripBlockProps>) => onChange({ ...props, ...patch });
  ...
  const addImage = () =>
    update({ images: [...images, { id: `${Date.now()}`, src: "", caption: "New photo", alt: "" }] });
    ...
    <div className="space-y-5">
      <div className="space-y-3">
        ...
        <div>
          ...
          <AiTextField value={props.headline} onChange={(v) => update({ headline: v })} rows={2} className="text-xs" onSuggest={() => suggestCopy("gallery-filmstrip", "headline", props.headline ?? "", {})} fieldLabel="Headline" />
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {images.map((img, i) => (
          <div key={img.id} className="border rounded-md p-3 space-y-2">
            ...
            <ImagePicker value={img.src} onChange={(src) => updateImage(i, { src })} label="Image" aiHint={img.caption || "Event photo"} />
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <AiTextField type="input" value={props.ctaLabel ?? ""} onChange={(v) => update({ ctaLabel: v })} className="h-8 text-xs" placeholder="Leave blank to hide" onSuggest={() => suggestCopy("gallery-filmstrip", "ctaLabel", props.ctaLabel ?? "", { headline: props.headline ?? "" })} fieldLabel="Link label" />
        </div>
        <div>
