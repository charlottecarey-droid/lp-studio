import type { TestimonialGridBlockProps, TestimonialGridItem } from "@/lib/block-types";
...
interface Props {
  ...
  onChange: (next: TestimonialGridBlockProps) => void;
}

export function TestimonialGridPanel({ props, onChange }: Props) {
  const update = (patch: Partial<TestimonialGridBlockProps>) => onChange({ ...props, ...patch });
  const items = props.testimonials ?? [];
  const updateItem = (i: number, patch: Partial<TestimonialGridItem>) =>
    ...
    <div className="space-y-5">
      <div className="space-y-3">
        ...
        <div>
          ...
          <AiTextField type="input" value={props.eyebrow ?? ""} onChange={(v) => update({ eyebrow: v })} className="h-8 text-xs" onSuggest={() => suggestCopy("testimonial-grid", "eyebrow", props.eyebrow ?? "", { headline: props.headline ?? "" })} fieldLabel="Eyebrow" />
        </div>
        <div>
          <Label className="text-[11px] text-muted-foreground">Headline</Label>
          <AiTextField value={props.headline} onChange={(v) => update({ headline: v })} rows={2} className="text-xs" onSuggest={() => suggestCopy("testimonial-grid", "headline", props.headline ?? "", { eyebrow: props.eyebrow ?? "", subheadline: props.subheadline ?? "" })} fieldLabel="Headline" />
        </div>
        <div>
          <Label className="text-[11px] text-muted-foreground">Subheadline</Label>
          <AiTextField value={props.subheadline ?? ""} onChange={(v) => update({ subheadline: v })} rows={3} className="text-xs" placeholder="Leave blank to hide" onSuggest={() => suggestCopy("testimonial-grid", "subheadline", props.subheadline ?? "", { headline: props.headline ?? "" })} fieldLabel="Subheadline" />
        </div>
      </div>
        ...
        {items.map((t, i) => (
          <div key={t.id ?? i} className="border rounded-md p-3 space-y-2">
            ...
            <div>
              ...
              <AiTextField value={t.quote} onChange={(v) => updateItem(i, { quote: v })} rows={3} className="text-xs" onSuggest={() => suggestCopy("testimonial-grid", "quote", t.quote ?? "", { author: t.author ?? "", company: t.company ?? "" })} fieldLabel="Quote" />
            </div>
      ...
      <BenefitsCtaSection props={props} update={update} blockType="testimonial-grid" />
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <ColorField label="Card background" value={props.cardBgColor} onChange={(v) => update({ cardBgColor: v })} />
        </div>
      </div>
