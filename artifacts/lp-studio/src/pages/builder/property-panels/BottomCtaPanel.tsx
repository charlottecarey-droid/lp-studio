...
/** The bottom-CTA renderer (BlockBottomCta) only honors these primary actions. */
const BOTTOM_CTA_ACTIONS = ["url", "chilipiper", "modal-form", "modal-chilipiper"] as const;
...
export function BottomCtaPanel({ blockType, props, onChange, brandVoiceSet, onApplyCtaToAll, ctaSource }: Props) {
  return (
    <div className="space-y-4">
      ...
      {/* Shared CTA action + modal suite (Phase 2). Button label stays above. */}
      <CtaActionConfigSection
        value={props as CtaSuiteFields}
        onChange={(v) => onChange({ ...props, ...v } as BottomCtaBlockProps)}
        allowedActions={BOTTOM_CTA_ACTIONS}
        {...ctaSource}
      />
      {onApplyCtaToAll && (
        <button
