import type { HowItWorksVerticalTimelineBlockProps, HowItWorksVerticalTimelineStep } from "@/lib/block-types";
...
interface Props {
  ...
  onChange: (next: HowItWorksVerticalTimelineBlockProps) => void;
}

export function HowItWorksVerticalTimelinePanel({ props, onChange }: Props) {
  const update = (patch: Partial<HowItWorksVerticalTimelineBlockProps>) => onChange({ ...props, ...patch });
  const steps = props.steps ?? [];
  const updateStep = (i: number, patch: Partial<HowItWorksVerticalTimelineStep>) =>
  ...
  return (
    <div className="space-y-5">
      <div className="space-y-3">
        ...
        <BlockRefreshButton
          blockType="how-it-works-vertical-timeline"
        ...
        <div>
          ...
          <AiTextField type="input" value={props.eyebrow ?? ""} onChange={(v) => update({ eyebrow: v })} className="h-8 text-xs" onSuggest={() => suggestCopy("how-it-works-vertical-timeline", "eyebrow", props.eyebrow ?? "", { headline: props.headline ?? "" })} fieldLabel="Eyebrow" />
        </div>
        <div>
          <Label className="text-[11px] text-muted-foreground">Headline</Label>
          <AiTextField value={props.headline} onChange={(v) => update({ headline: v })} rows={2} className="text-xs" onSuggest={() => suggestCopy("how-it-works-vertical-timeline", "headline", props.headline ?? "", { eyebrow: props.eyebrow ?? "", subheadline: props.subheadline ?? "" })} fieldLabel="Headline" />
        </div>
        <div>
          <Label className="text-[11px] text-muted-foreground">Subheadline</Label>
          <AiTextField value={props.subheadline ?? ""} onChange={(v) => update({ subheadline: v })} rows={3} className="text-xs" placeholder="Leave blank to hide" onSuggest={() => suggestCopy("how-it-works-vertical-timeline", "subheadline", props.subheadline ?? "", { headline: props.headline ?? "" })} fieldLabel="Subheadline" />
        </div>
      </div>
        ...
        {steps.map((step, i) => (
          <div key={i} className="border rounded-md p-3 space-y-2">
            ...
            <div>
              ...
              <Input value={step.icon} onChange={(e) => updateStep(i, { icon: e.target.value })} placeholder="Palette" className="h-8 text-xs" />
            </div>
            <div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            ...
            <Input value={props.primaryButtonLabel ?? ""} onChange={(e) => update({ primaryButtonLabel: e.target.value })} placeholder="Leave blank to hide" className="h-8 text-xs" />
          </div>
          <div>
            <Label className="text-[11px] text-muted-foreground">Primary URL</Label>
            <Input value={props.primaryButtonUrl ?? ""} onChange={(e) => update({ primaryButtonUrl: e.target.value })} placeholder="#" className="h-8 text-xs" />
          </div>
          <div>
            <Label className="text-[11px] text-muted-foreground">Secondary label</Label>
            <Input value={props.secondaryButtonLabel ?? ""} onChange={(e) => update({ secondaryButtonLabel: e.target.value })} placeholder="Leave blank to hide" className="h-8 text-xs" />
          </div>
          <div>
            <Label className="text-[11px] text-muted-foreground">Secondary URL</Label>
            <Input value={props.secondaryButtonUrl ?? ""} onChange={(e) => update({ secondaryButtonUrl: e.target.value })} placeholder="#" className="h-8 text-xs" />
          </div>
        </div>
      </div>

      <BenefitsCtaSection props={props} update={update} blockType="how-it-works-vertical-timeline" />
