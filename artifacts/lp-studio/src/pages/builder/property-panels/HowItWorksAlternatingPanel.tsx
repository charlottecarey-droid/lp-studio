import type { HowItWorksAlternatingBlockProps, HowItWorksAlternatingStep } from "@/lib/block-types";
...
interface Props {
  ...
  onChange: (next: HowItWorksAlternatingBlockProps) => void;
}

export function HowItWorksAlternatingPanel({ props, onChange }: Props) {
  const update = (patch: Partial<HowItWorksAlternatingBlockProps>) => onChange({ ...props, ...patch });
  const steps = props.steps ?? [];
  const updateStep = (i: number, patch: Partial<HowItWorksAlternatingStep>) =>
  ...
  return (
    <div className="space-y-5">
      <div className="space-y-3">
        ...
        <BlockRefreshButton
          blockType="how-it-works-alternating"
        ...
        <div>
          ...
          <AiTextField type="input" value={props.eyebrow ?? ""} onChange={(v) => update({ eyebrow: v })} className="h-8 text-xs" onSuggest={() => suggestCopy("how-it-works-alternating", "eyebrow", props.eyebrow ?? "", { headline: props.headline ?? "" })} fieldLabel="Eyebrow" />
        </div>
        <div>
          <Label className="text-[11px] text-muted-foreground">Headline</Label>
          <AiTextField value={props.headline} onChange={(v) => update({ headline: v })} rows={2} className="text-xs" onSuggest={() => suggestCopy("how-it-works-alternating", "headline", props.headline ?? "", { eyebrow: props.eyebrow ?? "", subheadline: props.subheadline ?? "" })} fieldLabel="Headline" />
        </div>
        <div>
          <Label className="text-[11px] text-muted-foreground">Subheadline</Label>
          <AiTextField value={props.subheadline ?? ""} onChange={(v) => update({ subheadline: v })} rows={3} className="text-xs" placeholder="Leave blank to hide" onSuggest={() => suggestCopy("how-it-works-alternating", "subheadline", props.subheadline ?? "", { headline: props.headline ?? "" })} fieldLabel="Subheadline" />
        </div>
      </div>
        ...
        {steps.map((step, i) => (
          <div key={i} className="border rounded-md p-3 space-y-2">
            ...
            <div>
              ...
              <Input value={step.icon} onChange={(e) => updateStep(i, { icon: e.target.value })} placeholder="LayoutTemplate" className="h-8 text-xs" />
            </div>
            <div>
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) => updateStep(i, { features: e.target.value.split("\n").map((s) => s.trim()).filter(Boolean) })}
                rows={3}
      ...
      <BenefitsCtaSection props={props} update={update} blockType="how-it-works-alternating" />
