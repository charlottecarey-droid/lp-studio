import type { BenefitsBentoBlockProps, BenefitsBentoTile } from "@/lib/block-types";
...
interface Props {
  ...
  onChange: (next: BenefitsBentoBlockProps) => void;
}
    ...
    <div className="space-y-5">
      <div className="space-y-3">
        ...
        <div>
          ...
          <AiTextField type="input" value={props.eyebrow ?? ""} onChange={(v) => update({ eyebrow: v })} className="h-8 text-xs" onSuggest={() => suggestCopy("benefits-bento", "eyebrow", props.eyebrow ?? "", { headline: props.headline ?? "" })} fieldLabel="Eyebrow" />
        </div>
        <div>
          <Label className="text-[11px] text-muted-foreground">Headline</Label>
          <AiTextField value={props.headline} onChange={(v) => update({ headline: v })} rows={2} className="text-xs" onSuggest={() => suggestCopy("benefits-bento", "headline", props.headline ?? "", { eyebrow: props.eyebrow ?? "", subheadline: props.subheadline ?? "" })} fieldLabel="Headline" />
        </div>
        <div>
          <Label className="text-[11px] text-muted-foreground">Subheadline</Label>
          <AiTextField value={props.subheadline ?? ""} onChange={(v) => update({ subheadline: v })} rows={3} className="text-xs" placeholder="Leave blank to hide" onSuggest={() => suggestCopy("benefits-bento", "subheadline", props.subheadline ?? "", { headline: props.headline ?? "" })} fieldLabel="Subheadline" />
        </div>
      </div>
        ...
        {tiles.map((tile, i) => (
          <div key={i} className="border rounded-md p-3 space-y-2">
            <div className="flex justify-between items-center">
              <span className="text-xs font-medium">Tile {i + 1}</span>
            ...
            <div>
              ...
              <Input value={tile.icon} onChange={(e) => updateTile(i, { icon: e.target.value })} placeholder="Layers" className="h-8 text-xs" />
            </div>
            <div>
      ...
      <BenefitsCtaSection props={props} update={update} blockType="benefits-bento" />
