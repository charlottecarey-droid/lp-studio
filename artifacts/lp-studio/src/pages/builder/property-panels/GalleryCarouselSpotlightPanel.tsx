import type { GalleryCarouselSpotlightBlockProps, GalleryImage } from "@/lib/block-types";
...
interface Props {
  ...
  onChange: (next: GalleryCarouselSpotlightBlockProps) => void;
}

export function GalleryCarouselSpotlightPanel({ props, onChange }: Props) {
  const update = (patch: Partial<GalleryCarouselSpotlightBlockProps>) => onChange({ ...props, ...patch });
  ...
  const addImage = () =>
    update({ images: [...images, { id: `${Date.now()}`, src: "", caption: "New slide", alt: "" }] });
    ...
    <div className="space-y-5">
      <div className="space-y-3">
        ...
        <div>
          ...
          <AiTextField type="input" value={props.eyebrow ?? ""} onChange={(v) => update({ eyebrow: v })} className="h-8 text-xs" onSuggest={() => suggestCopy("gallery-carousel-spotlight", "eyebrow", props.eyebrow ?? "", { headline: props.headline ?? "" })} fieldLabel="Eyebrow" />
        </div>
        <div>
          <Label className="text-[11px] text-muted-foreground">Headline</Label>
          <AiTextField value={props.headline} onChange={(v) => update({ headline: v })} rows={2} className="text-xs" onSuggest={() => suggestCopy("gallery-carousel-spotlight", "headline", props.headline ?? "", { eyebrow: props.eyebrow ?? "", subheadline: props.subheadline ?? "" })} fieldLabel="Headline" />
        </div>
        <div>
          <Label className="text-[11px] text-muted-foreground">Subheadline</Label>
          <AiTextField value={props.subheadline ?? ""} onChange={(v) => update({ subheadline: v })} rows={3} className="text-xs" placeholder="Leave blank to hide" onSuggest={() => suggestCopy("gallery-carousel-spotlight", "subheadline", props.subheadline ?? "", { headline: props.headline ?? "" })} fieldLabel="Subheadline" />
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {images.map((img, i) => (
          <div key={img.id} className="border rounded-md p-3 space-y-2">
            ...
            <ImagePicker value={img.src} onChange={(src) => updateImage(i, { src })} label="Image" aiHint={img.caption || "Product screenshot"} />
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <AiTextField type="input" value={props.ctaLabel ?? ""} onChange={(v) => update({ ctaLabel: v })} className="h-8 text-xs" placeholder="Leave blank to hide" onSuggest={() => suggestCopy("gallery-carousel-spotlight", "ctaLabel", props.ctaLabel ?? "", { headline: props.headline ?? "" })} fieldLabel="Button label" />
        </div>
        <div>
