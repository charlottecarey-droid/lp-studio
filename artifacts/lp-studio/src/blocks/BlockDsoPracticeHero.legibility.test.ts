...
/**
 * Accent-legibility regression guard for the dark `dso-practice-hero` variant,
 * mirroring `BlockDsoHeartlandHero.legibility.test.ts`.
 *
 * On the dark hero variant (the "dark" / brand-primary preset, or a cover image
 * under the scrim) the eyebrow and the primary CTA fill used to be painted with
 * the raw brand accent (`var(--brand-accent)` / `brand.accentColor`). When a
 * tenant's brand accent is itself dark those elements vanished into the hero.
 *
 ...
 * fill stands off the dark hero, and the CTA label contrasts its own fill; and
 * (b) a BRIGHT brand accent and assert it is passed through unchanged.
 * Thresholds are hardcoded so the contract fails loudly if a future change
 * drops the contrast guard.
 *
...
import type { DsoPracticeHeroBlockProps } from "@/lib/block-types/dso-blocks";
...
function darkProps(): DsoPracticeHeroBlockProps {
  return {
    eyebrow: EYEBROW,
    ...
    // Force the dark variant: the "dark" preset resolves to a dark surface.
...
function renderWithAccent(accentColor: string): string {
  ...
  return renderToStaticMarkup(
    createElement(BlockDsoPracticeHero, { props: darkProps(), brand }),
  );
}
...
/**
 * The eyebrow color sits on the OUTER pill `<span>`; the text is rendered by a
 * nested InlineText `<span>`. Capture the wrapper's inline style.
 */
function eyebrowColor(markup: string): string | undefined {
  const m = new RegExp(
    `<span\\b[^>]*\\sstyle="([^"]*)"[^>]*>\\s*<span\\b[^>]*>${EYEBROW}<`,
    "i",
  ).exec(markup);
  expect(m, "expected the eyebrow pill element").not.toBeNull();
  return colorOf(m![1]);
}
 ...
 * uses `background-color`; the progress/icon controls set neither both a fill
 * AND a text color, so only the real CTA is returned.
 */
...
describe("BlockDsoPracticeHero accent legibility on the dark hero", () => {
