        ...
        <div className="grid md:grid-cols-3 gap-12 relative">
          ...
          {props.steps.map((step, i) => (
            <div key={i} className="relative z-10 flex flex-col items-center text-center">
              <div className="w-16 h-16 rounded-full font-display font-bold text-2xl flex items-center justify-center mb-6 shadow-xl border-4 border-white" style={{ backgroundColor: props.circleBg ?? "var(--brand-primary)", color: props.circleText ?? "var(--brand-accent)" }}>
                {step.number}
              </div>
