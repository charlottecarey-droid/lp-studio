      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10 py-20">
        <div className="grid md:grid-cols-[240px_1fr] gap-14 mb-14">
          ...
          <div className={cn("grid gap-10", groups.length <= 2 ? "grid-cols-2" : groups.length === 3 ? "grid-cols-3" : "grid-cols-4")}>
            {groups.map((group, i) => (
              <div key={i}>
                <span className="text-xs font-bold uppercase tracking-widest text-[#003A30] block mb-5">
                  {group.heading}
                </span>
                <ul className="space-y-3">
                  {(group.links ?? []).map((link, j) => (
                    <li key={j}>
                      <a
                        href={link.url || "#"}
                        className="text-base text-slate-500 hover:text-[#003A30] transition-colors"
                      >
                        {link.label}
                      </a>
                    </li>
        ...
        <div className="border-t border-slate-200 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            {props.facebookUrl && (
              <a href={props.facebookUrl} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center hover:border-[#003A30] transition-colors">
                <FacebookIcon />
              </a>
            )}
            {props.instagramUrl && (
              <a href={props.instagramUrl} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center hover:border-[#003A30] transition-colors">
                <InstagramIcon />
              </a>
            )}
            {props.linkedinUrl && (
              <a href={props.linkedinUrl} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center hover:border-[#003A30] transition-colors">
                <LinkedInIcon />
              </a>
            )}
          </div>
